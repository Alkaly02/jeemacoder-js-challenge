...
function addUser(users) {
    ...
    const displayUsers = users?.map(user => (
        `<tr>
            ...
            <td style="cursor: pointer" onclick="edit(${user.id})"><button class="btn border bg-warning">Modifier</button></td>
            <td style="cursor: pointer" onclick="deleteUser(${user.id})" id=${user.id}><button class="btn border bg-danger text-light">Supprimer</button></td>
        </tr>`
    ))
